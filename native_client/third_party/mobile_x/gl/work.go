...
uintptr_t process_X(struct fnargs* cargs, char* parg0, char* parg1, char* parg2, int count) {
	uintptr_t ret;

	ret = process_XFn(&cargs[0], parg0);
	if (count > 1) {
		ret = process_XFn(&cargs[1], parg1);
	}
	if (count > 2) {
		ret = process_XFn(&cargs[2], parg2);
	}
...
func (ctx *context) DoWork() {
	...
	enqueue:
		...
		ret := C.process_X(&ctx.cargs[0], ctx.parg[0], ctx.parg[1], ctx.parg[2], C.int(len(queue)))
