...
func NewAnimatedEmoji(shortcut string, slicer *AeroSlicer) *AnimatedEmoji {
	// Prefer standalone PNGs (generated by cmd/emoticongen) so chat + picker work without a vendor spritesheet.
	if rel, ok := EmoticonFileForShortcut(shortcut); ok {
		if raw, ok := ReadAssetRaw(rel); ok {
			res := fyne.NewStaticResource(rel, raw)
			return newStaticAnimatedEmoji(res, shortcut)
		}
	}
	...
	first := slicer.Slice(fmt.Sprintf("%s_%d", shortcut, 0), coords.X, coords.Y, 40, 40)
	if first == nil {
		return nil
	}
	...
	e := &AnimatedEmoji{
		...
		slicer:   slicer,
		shortcut: shortcut,
		imageObj: canvas.NewImageFromResource(first),
	}
	...
	if e.frames > 1 {
		go func() {
			...
			for range ticker.C {
				...
				res := slicer.Slice(fmt.Sprintf("%s_%d", shortcut, e.current), coords.X, coords.Y+(e.current*40), 40, 40)
				if res != nil {
					e.imageObj.Resource = res
					e.imageObj.Refresh()
				}
			}
		}()
...
func newStaticAnimatedEmoji(res fyne.Resource, shortcut string) *AnimatedEmoji {
	e := &AnimatedEmoji{
		frames:   1,
		shortcut: shortcut,
		imageObj: canvas.NewImageFromResource(res),
	}
	e.imageObj.FillMode = canvas.ImageFillContain
	e.ExtendBaseWidget(e)
	return e
